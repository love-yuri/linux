...
/* get_task_state(): */
#define TASK_REPORT \
  (TASK_RUNNING | TASK_INTERRUPTIBLE | TASK_UNINTERRUPTIBLE | __TASK_STOPPED | __TASK_TRACED | EXIT_DEAD | EXIT_ZOMBIE | TASK_PARKED)
...
/*
 * PREEMPT_RT “睡眠”自旋/rwlock 的特定变体
 *
 *RT 的 spin/rwlock 替换是状态保留的。的状态
 *锁上阻塞时的任务保存在task_struct::saved_state中并且
 *获取锁后恢复。这些操作是
 *由task_struct::pi_lock 针对try_to_wake_up() 进行序列化。任何非 RT
 *当任务被锁阻塞时，与锁相关的唤醒是
 *重定向到对task_struct::saved_state进行操作以确保这些
 *没有被丢弃。恢复时，task_struct::saved_state 设置为
 *TASK_RUNNING 因此任何重定向到saved_state 的唤醒尝试都将失败。
 *
